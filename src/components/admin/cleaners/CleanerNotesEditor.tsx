...
import { saveCleanerNotes } from '@/app/(admin)/cleaners/actions'
...
export default function CleanerNotesEditor({ cleanerId, initialNotes, updatedAt }: Props) {
  const [notes, setNotes] = useState(initialNotes ?? '')
  const [saved, setSaved] = useState(false)
  const timer = useRef<ReturnType<typeof setTimeout> | null>(null)

  async function handleBlur() {
    await saveCleanerNotes(cleanerId, notes)
    setSaved(true)
